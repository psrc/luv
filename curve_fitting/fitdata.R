...
x1 <- seq(2010, 2040) # values on the x-axis
...
#rownam <- colnames(all.data)[3:ncol(all.data)]
set.seed(1) # makes it reproducible
for(iarea in 1:nrow(all.data)) { # iterate over each area
	this.row <- as.numeric(all.data[iarea,3:ncol(all.data)])
	names(this.row) <- colnames(all.data)[3:ncol(all.data)]
	idx <- which(!is.na(this.row))
	x <- unique(x.all[!is.na(this.row)])
	xcol <- c(x[x<=2013], rep(x[x>2013],2))
	xcoln <- as.character(xcol)
	xcoln[3:length(xcoln)] <- paste0(xcoln[3:length(xcoln)], rep(c('low', 'high'), each=2))
	data <- data.frame(x=xcol, y=(this.row[idx])[xcoln])
	#rownames(data) <- rownam[idx]
	rownames2030 <- c(paste0(x[3],'low'), paste0(x[3], 'high'))
	rownames2040 <- c(paste0(x[4],'low'), paste0(x[4], 'high'))
	...
	for(i in 1:R) {
		y[3:4] <- c(sampl2030[idx1[i]], sampl2040[idx2[i]])
		splfun <- splinefun(x, y, method="natural")
		#f <- function(x) splfun(x, deriv=2)^2
		deriv2[i] <- mean(abs(splfun(x1, deriv=2))[x1>2013 & x1<2040])
		deriv1[i] <- mean(splfun(x1, deriv=1)[x1>2013])
	...
	y <- c(data$y[1:2], mean(data[rownames2030,'y']), mean(data[rownames2040,'y']))
	...
	plot(x[1:2], y[1:2], main=all.data[iarea,'Jurisdiction'], 
