...
def get_indicators(cache_directory, run_description, years = [2023,2025,2030,2035,2040,2044,2050], base_year=2023):
    ...
    indicators=[
     ...
     # Table(
        # attribute = 'households=subreg.aggregate(urbansim_parcel.building.number_of_households, intermediates=[parcel])',
            # dataset_name = 'subreg',
            # source_data = source_data,
            # ),
            # Table(
        # attribute = 'population=subreg.aggregate(urbansim_parcel.building.population, intermediates=[parcel])',
            # dataset_name = 'subreg',
            # source_data = source_data,
            # ),
             # Table(
        # attribute = 'employment=subreg.aggregate(urbansim_parcel.building.number_of_jobs, intermediates=[parcel])',
            # dataset_name = 'subreg',
            # source_data = source_data,
            # ),
            # Table(
        ...
        # attribute = 'acres=subreg.aggregate(parcel.parcel_sqft/43560.)',
           ...
           # ),
           # Table(
        # attribute =  'activity_units = subrege.aggregate(urbansim_parcel.parcel.population, intermediates=[parcel]) + subreg.aggregate(urbansim_parcel.parcel.number_of_jobs, intermediates=[parcel])',
           # dataset_name = 'subreg',
           # source_data = source_data,
           # ),
...
# Table(
       ...
       # attribute = 'activity_units = county.aggregate(urbansim_parcel.parcel.population, intermediates=[parcel]) + county.aggregate(urbansim_parcel.parcel.number_of_jobs, intermediates=[parcel])',
            ...
            # ),
            # Table(
        # attribute = 'nonres_sqft=county.aggregate(urbansim_parcel.building.non_residential_sqft, intermediates=[parcel])',
            # dataset_name = 'county',
            # source_data = source_data,
            # ),	   
            # Table(
        # attribute = 'residential_units=county.aggregate(urbansim_parcel.building.residential_units, intermediates=[parcel])',
            # dataset_name = 'county',
            # source_data = source_data,
            # ),	   

# County aggregation over targets table			
Table(
            attribute = 'residential_units=county.aggregate(urbansim_parcel.building.residential_units, intermediates=[target])',
            dataset_name = 'county',
            source_data = source_data,
            ),	   			
   ...
   # source_data = source_data,
    ...
    # Table(
        ...
        # source_data = source_data,
            # dataset_name = 'faz',
            # name = 'employment_by_aggr_sector',
            # attributes = jobs_by_luv_sector("faz", "urbansim_parcel"),
            # output_type = 'tab'
            # ),
