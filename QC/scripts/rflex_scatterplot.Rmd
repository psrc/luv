...
Select the 'Box Select' or 'Lasso Select' option in the scatterplot to select points and see its location on the map.
...
output$plot <- renderPlotly({
  ...
  key <- data$faz_id #uniquely identify geo for Plotly
...
# Shapefile
shape <- reactive({
  data1 <- subset(indicators.table, indicators.table$indicator==table()) 
  shape.join <- sp::merge(faz.shape, data1, by.x = c("FAZ10", "Name"), by.y = c("faz_id", "Name"))
  return (shape.join)
})
...
# Re-draw the map with selected tracts    
...
selected_geo <- reactive({
  ...
  if(is.null(eventdata)){
    ...
    geoid <- eventdata[['key']]
    sub <- shape()[shape()$nFAZ10 %in% geoid, ] # Give back a sp data frame of the selected name_ids
    return(sub)
  }
}) 
