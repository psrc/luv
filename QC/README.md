...
IF:  you get an error message, 'the requested operation requires elevation', try running make in Git Bash (if you were using Console Z or some other program) or run the program as administrator.  Christy & Mark got that error message using Console Z on MODELSRV8 but not in Git Bash - web search indicated it has to do with Admin rights.  
